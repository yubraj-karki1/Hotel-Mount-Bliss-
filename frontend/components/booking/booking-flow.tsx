...
export function BookingFlow() {
  ...
  const rooms = useQuery({ queryKey: ["rooms", "booking"], queryFn: async () => (await roomApi.list()).data.data.items });
  ...
  const selectedRoomBookable = Boolean(selectedRoom && !["MAINTENANCE", "OUT_OF_SERVICE"].includes(selectedRoom.status));
  ...
  return <Card><CardContent className="grid gap-5 p-6 sm:p-8">
    ...
    <label className="grid gap-2 text-sm font-semibold">Room<select className="h-11 rounded-md border bg-card px-3" value={form.roomId} onChange={event => update("roomId", event.target.value)}><option value="">Select a room</option>{rooms.data?.map(room => <option key={room._id} value={room._id}>{room.name} — {formatCurrency(room.price)}/night{["MAINTENANCE", "OUT_OF_SERVICE"].includes(room.status) ? " (currently unavailable)" : ""}</option>)}</select></label>
