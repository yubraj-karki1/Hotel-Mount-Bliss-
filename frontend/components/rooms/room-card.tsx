...
export function RoomCard({ room }: { room: Room }) {
  ...
  return <Card className="group overflow-hidden">
    <div className="relative aspect-[4/3] overflow-hidden bg-secondary">
      ...
      <Badge className="absolute left-4 top-4" variant={available ? "accent" : "destructive"}>{available ? room.type : statusLabel}</Badge>
    ...
    <CardContent className="p-5">
      ...
      <div className="mt-6 flex items-end justify-between gap-3">
        ...
        <div className="flex gap-2">
          ...
          {available ? <Button size="sm" asChild><Link href={`/booking?room=${id}`}>Book</Link></Button> : <Button size="sm" disabled>{statusLabel}</Button>}
        </div>
      </div>
    </CardContent>
  </Card>;
