...
const roomImage = "/images/gallery/guest-room.webp";

export function RoomCard({ room }: { room: Room }) {
  const id = room._id ?? room.id!;
  const image = room.images?.[0] ?? roomImage;
  const available = !["MAINTENANCE", "OUT_OF_SERVICE"].includes(room.status);
  return <Card className="group overflow-hidden">
    <div className="relative aspect-[4/3] overflow-hidden bg-secondary">
      <Image src={image} alt={`${room.name} interior`} fill loading="eager" sizes="(max-width: 768px) 100vw, (max-width: 1280px) 50vw, 33vw" className="object-cover transition-transform duration-500 group-hover:scale-[1.03]" />
