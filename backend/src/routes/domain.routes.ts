...
router.post("/admin/staff", authenticate, managers, validate(z.object({ body: z.object({ name: z.string().trim().min(2), email: z.email().transform(v=>v.toLowerCase()), phone: z.string().trim().min(7), password: z.string().min(8), role: z.enum(["RECEPTIONIST", "HOUSEKEEPER", "MANAGER", "ADMIN"]) }) })), asyncHandler(async (req, res) => { if(await User.exists({email:req.body.email})) throw new AppError(409,"Email already exists"); const user=await User.create({...req.body,password:await bcrypt.hash(req.body.password,12),isEmailVerified:true}); return success(res,201,"Staff account created",user); }));
router.patch("/admin/staff/:id", authenticate, managers, validate(z.object({ params:z.object({id:objectId}), body:z.object({name:z.string().trim().min(2),phone:z.string().trim().min(7),role:z.enum(["RECEPTIONIST","HOUSEKEEPER","MANAGER","ADMIN"]),isActive:z.boolean()}).partial() })), asyncHandler(async(req,res)=>{const user=await User.findOneAndUpdate({_id:req.params.id,role:{$ne:"CUSTOMER"}},req.body,{new:true,runValidators:true});if(!user)throw new AppError(404,"Staff member not found");return success(res,200,"Staff updated",user)}));
