...
router.post("/contact", validate(z.object({ body: z.object({ name: z.string().trim().min(2).max(100), email: z.email().transform(v => v.toLowerCase()), subject: z.string().trim().min(3).max(200), message: z.string().trim().min(10).max(5000) }) })), asyncHandler(async (req, res) => {
  const inquiry = await Inquiry.create(req.body);
  // The enquiry is the primary operation. A notification must never turn a saved
  // enquiry into a failed customer submission.
  void notifyStaff("New enquiry", `${inquiry.name}: ${inquiry.subject}`, "INQUIRY").catch(error => req.log.error({ err: error, inquiryId: inquiry.id }, "Could not create enquiry notifications"));
  return success(res, 201, "Enquiry received", inquiry);
}));
