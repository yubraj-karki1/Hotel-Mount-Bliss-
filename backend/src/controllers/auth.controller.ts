...
export const authController = {
  ...
  login: async (req: Request, res: Response) => { const result = await authService.login(req.body); res.cookie("accessToken", result.accessToken, cookieOptions(req.body.rememberMe)); await auditService.record({ user: result.user.id, action: "LOGIN", resource: "User", resourceId: result.user.id, description: req.body.rememberMe ? "User logged in with a remembered session" : "User logged in", ...context(req) }); return success(res, 200, "Login successful", { user: result.user }); },
  logout: async (req: Request, res: Response) => { res.clearCookie("accessToken", clearCookieOptions); if (req.auth) await auditService.record({ user: req.auth.userId, action: "LOGOUT", resource: "User", resourceId: req.auth.userId, description: "User logged out", ...context(req) }); return success(res, 200, "Logout successful", null); },
  ...
  changePassword: async (req: Request, res: Response) => { await authService.changePassword(req.auth!.userId, req.body.currentPassword, req.body.newPassword); res.clearCookie("accessToken", clearCookieOptions); return success(res, 200, "Password changed; please log in again", null); },
};
